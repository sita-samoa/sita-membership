...
function update() {
  form.put(
    route("members.documents.update", {
...
function deleteItem() {
  form.delete(
    route("members.documents.destroy", {
      member: props.member_id,
      document: itemId.value,
    }),
    {
...
<template>
  <div>
    <Alert type="info" class="my-2"
      >Please attach certified copies of Supporting documents.
    </Alert>
    <h5>Supporting Documents</h5>

    <Button class="p-3 my-3" color="alternative" @click.prevent="showModal"
      >Add Document</Button
    >
  ...
  <DialogModal :show="showFormModal" @close="closeModalAndResetForm">
    ...
    <template #content>
      <Input
        v-model="form.title"
        :placeholder="form.file_name ? form.file_name : 'enter your title'"
        label="Title"
        class="mb-2"
      />
