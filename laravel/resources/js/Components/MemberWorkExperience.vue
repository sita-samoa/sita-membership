...
<template>
        ...
        <tbody>
          <tr v-for="workExperience in memberWorkExperiences" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
            <td class="px-6 py-4 text-right">
              <button type="button" @click="showModal(workExperience)" class="font-medium text-blue-600 dark:text-blue-500 hover:underline"><PencilOutlineIcon fillColor="green" /></button>
            </td>
  ...
  <DialogModal :show="isShowModal" @close="closeModal">
    ...
    <template #content>
      ...
      <Input id="organisation" ref="organisation" type="text" placeholder="enter your organisation" v-model="form.organisation" />
      <InputError class="mt-2 mb-4" :message="form.errors.organisation" />

      <!-- Position -->
      <InputLabel for="position" value="Position" class="mb-2 required" />
      <Input id="position" placeholder="enter your position" label="Position" class="mb-2" v-model="form.position" />
      ...
      <div class="relative max-w-sm mb-3">
        <input type="date" id="from_date" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Select date" v-model="form.from_date" />
      </div>
      ...
      <div class="relative max-w-sm mb-3">
        <input type="date" id="to_date" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Select date" v-model="form.to_date" />
      </div>
    ...
    <template #footer>
      ...
      <div>
        ...
        <button type="submit" @click="addSaveWorkExperience" class="ml-2 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
          {{ !form.id ? 'Add' : 'Update' }}
        </button>
      </div>
    </template>
