...
function update() {
  form.put(
    route("members.qualifications.update", {
      member: props.member_id,
      qualification: itemId.value,
    }),
    {
      preserveScroll: true,
...
function deleteItem() {
  form.delete(
    route("members.qualifications.destroy", {
      member: props.member_id,
      qualification: itemId.value,
    }),
    {
  ...
  <DialogModal :show="showFormModal" @close="closeModalAndResetForm">
    ...
    <template #content>
      ...
      <Input
        name="year_attained"
        type="number"
        v-model="form.year_attained"
        placeholder="enter your year attended"
        label="Year attended"
        class="mb-2"
      />
